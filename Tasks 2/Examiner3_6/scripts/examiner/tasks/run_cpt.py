...
"""Script example for running the Continuous Performance Task
    
  Provides example of loading configuration file, running the environment check, 
  gathering input from tester with Default Values, and running the Comtinuous Performance Task   
...
sessionConfig = lavatask.base.SessionConfig("Configure CPT Session")
if(not sessionConfig.getSessionConfig()):
    core.quit()
    
#Setup default task execution data
task = examiner.cpt.CPTTask(config,environment,sessionConfig)
task.runTask()
core.quit()
sys.exit()
