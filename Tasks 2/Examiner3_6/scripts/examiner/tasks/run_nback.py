...
"""Script example for running the NBack Task
    
  Provides example of loading configuration file, running the environment check, 
  gathering input from tester with Default Values, and running the NBack Task   
...
sessionConfig = lavatask.base.SessionConfig("Configure NBack Session")
if(not sessionConfig.getSessionConfig()):
    core.quit()

#Setup default task execution data
task = examiner.nback.NBackTask(config,environment,sessionConfig)
task.runTask()
core.quit()
sys.exit()
